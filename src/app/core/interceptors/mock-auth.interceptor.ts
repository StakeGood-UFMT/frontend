...
export const mockAuthInterceptor: HttpInterceptorFn = (req: HttpRequest<unknown>, next: HttpHandlerFn): Observable<HttpEvent<unknown>> => {
  // Only intercept if the URL contains the auth base path
  ...
  // 1. GET /nonce
  if (req.method === 'GET' && req.url.endsWith(API_CONFIG.endpoints.auth.nonce)) {
  ...
  // 2. POST /verify
  if (req.method === 'POST' && req.url.endsWith(API_CONFIG.endpoints.auth.verify)) {
  ...
  // 3. POST /refresh
  if (req.method === 'POST' && req.url.endsWith(API_CONFIG.endpoints.auth.refresh)) {
    const payload = btoa(JSON.stringify({
